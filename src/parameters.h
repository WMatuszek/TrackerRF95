...
uint16_t inline getFlashSize(void) {
    return *FlashSize;
}

#define UniqueID ((uint32_t *)(0x1FFFF7E8))
uint32_t inline getUniqueID(uint8_t Idx) {
    return UniqueID[Idx];
}
...
class FlashParameters {
 public:
    ...
    int8_t getTxPower(void) const {
        int8_t Pwr = RFchipTxPower & 0x7F;
        if (Pwr & 0x40) Pwr |= 0x80;
        return Pwr;
    }
 ...
 public:
    ...
    int8_t WriteToFlash(uint32_t *Addr = 0) const  // write parameters to Flash
    {
        ...
        {
            FLASH_ProgramWord((uint32_t)Addr, Data[Idx]);
            Addr++;
        }                                                          // !=FLASH_COMPLETE ?    // write to Flash
    ...
    {
        uint8_t Len = 0;
        Line[Len++] = HexDigit(getAcftType());
        Line[Len++] = ':';
        Line[Len++] = HexDigit(getAddrType());
        Line[Len++] = ':';
        ...
        Line[Len++] = ' ';
        Len += Format_SignDec(Line + Len, ((int32_t)RFchipFreqCorr * 15625 + 128) >> 8, 1);
        Len += Format_String(Line + Len, "Hz");
