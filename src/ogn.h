 ...
 public:
    ...
    void Dump(void) const {
        printf("%08lX: %08lX %08lX %08lX %08lX [%08lX %04lX] (%d)\n", (long int)Header, (long int)Position[0], (long int)Position[1],
               (long int)Position[2], (long int)Position[3], (long int)FEC[0], (long int)FEC[1], (int)checkFEC());
    }
    ...
    void Print(void) const {
        printf("%06lX:%c R%c %c%X %c", (long int)getAddress(), '0' + getAddrType(), '0' + getRelayCount(), isStealth() ? 's' : ' ',
               (int)getAcftType(), isEmergency() ? 'E' : ' ');
        printf("%d/%dD/%4.1f %02dsec: [%+10.6f, %+10.6f]deg %ldm %3.1fkt %05.1fdeg %+4.1fm/s %+4.1fdeg/s\n", (int)getFixQuality(),
               (int)getFixMode() + 2, 0.1 * (10 + DecodeDOP()), (int)getTime(), 0.0001 / 60 * DecodeLatitude(), 0.0001 / 60 * DecodeLongitude(),
               (long int)DecodeAltitude(), 0.2 * DecodeSpeed(), 0.1 * DecodeHeading(), 0.1 * DecodeClimbRate(), 0.1 * DecodeTurnRate());
    }
    ...
    uint8_t Print(char *Out) {
        ...
        Out[Len++] = ' ';
        Len += Format_UnsDec(Out + Len, (uint32_t)DecodeAltitude());
        Out[Len++] = 'm';
        Out[Len++] = ' ';
        Len += Format_UnsDec(Out + Len, DecodeSpeed() << 1, 2, 1);
        Out[Len++] = 'k';
        Out[Len++] = 't';
        Out[Len++] = ' ';
        Len += Format_SignDec(Out + Len, DecodeClimbRate(), 2, 1);
        Out[Len++] = 'm';
        Out[Len++] = '/';
        Out[Len++] = 's';
        Out[Len++] = '\n';
    ...
    void Whiten(void) {
        TEA_Encrypt_Key0(Position, 8);
        TEA_Encrypt_Key0(Position + 2, 8);
    }  // whiten the position
    void Dewhiten(void) {
        TEA_Decrypt_Key0(Position, 8);
        TEA_Decrypt_Key0(Position + 2, 8);
    }  // de-whiten the position

    int BitErr(OGN_Packet &RefPacket) const  // return number of different data bits between this Packet and RefPacket
    {
        return Count1s(Header ^ RefPacket.Header) + Count1s(Position[0] ^ RefPacket.Position[0]) + Count1s(Position[1] ^ RefPacket.Position[1]) +
               Count1s(Position[2] ^ RefPacket.Position[2]) + Count1s(Position[3] ^ RefPacket.Position[3]) + Count1s(FEC[0] ^ RefPacket.FEC[0]) +
    ...
    void calcAddrParity(void) {
        if (!goodAddrParity()) Header ^= 0x08000000;
    }  // if not correct parity, flip the parity bit

    bool isOther(void) const {
        return Header & 0x04000000;
    }  // this is a meteo or other report: sends wind speed/direction, pressure, temperatue and humidity
    ...
    void setStealth(uint8_t Stealth) {
        clrStealth();
    ...
    void calcRelayRank(int32_t RxAltitude)  // [0.1m] altitude of reception
    {
        if (isEmergency()) {
            Rank = 0xFF;
            return;
        }  // emergency packets always highest rank
        Rank = 0;
        ...
        if (RxAltitude > 0) Rank += RxAltitude >> 10;  // 1 point/100m of altitude below
        int16_t ClimbRate = DecodeClimbRate();         // [0.1m/s] higher sink rate => higher rank
        if (ClimbRate < 0) Rank += (-ClimbRate) >> 3;  // 1 point/0.8m/s of sink
    }

    void EncodeLatitude(int32_t Latitude)  // encode Latitude: units are 0.0001/60 degrees
    {
        Latitude >>= 3;
        Position[0] = (Position[0] & 0xFF000000) | (Latitude & 0x00FFFFFF);
    }  // scaled by 8 thus 1.5m resolution
    ...
    {
        Longitude >>= 4;
        Position[1] = (Position[1] & 0xFF000000) | (Longitude & 0x00FFFFFF);
    }  // scaled down by 16 thus 3m resolution on the equator
    ...
    void EncodeAltitude(int32_t Altitude)  // encode altitude in meters
    {
        if (Altitude < 0)
            ...
            Altitude = 0x1000 | ((Altitude - 0x1000) >> 1);
        else if (Altitude < 0x7000)
            Altitude = 0x2000 | ((Altitude - 0x3000) >> 2);
        else if (Altitude < 0xF000)
            Altitude = 0x3000 | ((Altitude - 0x7000) >> 3);
        else
            Altitude = 0x3FFF;
    ...
    int32_t DecodeAltitude(void) const  // return Altitude in meters
    {
        ...
        return 0x7004 + (Altitude << 3);
    }  // 7000..EFF8 => max. altitude: 61432 meters
    ...
    void EncodeSpeed(int16_t Speed)  // speed in 0.2 knots
    {
        if (Speed < 0)
            ...
            Speed = 0x100 | ((Speed - 0x100) >> 1);
        else if (Speed < 0x700)
            Speed = 0x200 | ((Speed - 0x300) >> 2);
        else if (Speed < 0xF00)
            Speed = 0x300 | ((Speed - 0x700) >> 3);
        else
            Speed = 0x3FF;
    ...
    int16_t DecodeSpeed(void) const  // return speed in 0.2 knots units (0.2 knots is about 0.1 m/s)
    {
        ...
        return 0x704 + (Speed << 3);
    }  // 700..EF8 => max. speed: 3832*0.2 = 766 knots
    ...
    void EncodeTurnRate(int16_t Turn) {
        ...
        if (Turn < 0x020) {
            ...
            Turn = 0x020 | ((Turn - 0x020) >> 1);
        else if (Turn < 0x0E0)
            Turn = 0x040 | ((Turn - 0x060) >> 2);
        else if (Turn < 0x1E0)
            Turn = 0x060 | ((Turn - 0x0E0) >> 3);
        else
            Turn = 0x07F;
        Turn |= Sign;
    ...
    int16_t DecodeTurnRate(void) const {
        ...
        if (Range == 0) {
            ...
            Turn = 0x0E4 + (Turn << 3);
        }  // 0E0..1D8 => max. turn rate = +/- 472*0.1 = +/- 47.2 deg/s
    ...
    void EncodeClimbRate(int16_t Climb) {
        ...
        if (Climb < 0x040) {
            ...
            Climb = 0x040 | ((Climb - 0x040) >> 1);
        else if (Climb < 0x1C0)
            Climb = 0x080 | ((Climb - 0x0C0) >> 2);
        else if (Climb < 0x3C0)
            Climb = 0x0C0 | ((Climb - 0x1C0) >> 3);
        else
            Climb = 0x0FF;
        Climb |= Sign;
    ...
    int16_t DecodeClimbRate(void) const {
        ...
        if (Range == 0) {
            ...
            Climb = 0x1C4 + (Climb << 3);
        }  // 1C0..3B8 => max. climb rate = +/- 952*0.1 = +/- 95.2 m/s
        return Sign ? -Climb : Climb;
    }

    void EncodeStdAltDiff(int8_t AltDiff) { Position[3] = (Position[3] & 0x00FFFFFF) | ((int32_t)AltDiff << 24); }

    int8_t DecodeStdAltDiff(void) const { return (Position[3] >> 24) & 0xFF; }
    ...
    */
    ...
    void TEA_Decrypt(uint32_t *Data, const uint32_t *Key, int Loops = 4) {
        uint32_t v0 = Data[0], v1 = Data[1];  // set up
        const uint32_t delta = 0x9e3779b9;
        uint32_t sum = delta * Loops;                                 // a key schedule constant
        uint32_t k0 = Key[0], k1 = Key[1], k2 = Key[2], k3 = Key[3];  // cache key
        for (int i = 0; i < Loops; i++)                               // basic cycle start */
        {
            v1 -= ((v0 << 4) + k2) ^ (v0 + sum) ^ ((v0 >> 5) + k3);
 ...
 public:
    ...
    void addNew(uint8_t NewIdx)  // add the new packet to the queue
    {
        ...
        for (uint8_t Idx = 0; Idx < Size; Idx++)                       // look for other packets with same ID
        {
            ...
            if (Packet[Idx].getAddressAndType() == AddressAndType)  // if another packet with same ID:
            {
                ...
                Low = 0;
                LowIdx = Idx;
            }  // then remove it: set rank to zero
        }
        uint8_t Rank = Packet[NewIdx].Rank;
        Sum += Rank;           // add the new packet to the rank sum
        if (NewIdx != LowIdx)  //
        {
            if (Rank <= Low) {
                Low = Rank;
    ...
    uint8_t Print(char *Out) {
        ...
        for (uint8_t Idx = 0; Idx < Size; Idx++) {
            ...
            if (Rank) {
                Out[Len++] = '/';
                Len += Format_Hex(Out + Len, Packet[Idx].getAddressAndType());
            }
        }
        Out[Len++] = ' ';
        Len += Format_Hex(Out + Len, Sum);
        Out[Len++] = '/';
        Len += Format_Hex(Out + Len, LowIdx);
        Out[Len++] = '\n';
        Out[Len] = 0;
        return Len;
    }
...
class OgnPosition {
 ...
 public:
    ...
    }  // all data must have been correctly read: negative means not correctly read)
    ...
    void copyTimeDate(OgnPosition &RefPosition) {
        copyTime(RefPosition);
        copyDate(RefPosition);
    }
    ...
    uint8_t incrTime(void)  // increment HH:MM:SS by one second
    {
        ...
        Hour = 0;
        return 1;
    }  // return 1 if date needs to be incremented
    ...
    void incrTimeData(void) {
        if (incrTime()) incrDate();
    }
    ...
    int PrintLine(char *Out) const {
        ...
        Out[Len++] = '/';
        Len += Format_UnsDec(Out + Len, PDOP, 2, 1);
        Out[Len++] = '/';
        Len += Format_UnsDec(Out + Len, HDOP, 2, 1);
        Out[Len++] = '/';
        Len += Format_UnsDec(Out + Len, VDOP, 2, 1);
        Out[Len++] = ' ';
        Out[Len++] = '[';
        Len += Format_SignDec(Out + Len, Latitude / 60, 6, 4);
        Out[Len++] = ',';
        Len += Format_SignDec(Out + Len, Longitude / 60, 7, 4);
        Out[Len++] = ']';
        Out[Len++] = 'd';
        Out[Len++] = 'e';
        Out[Len++] = 'g';
        Out[Len++] = ' ';
        Len += Format_SignDec(Out + Len, Altitude, 4, 1);
        Out[Len++] = 'm';
        Out[Len++] = '/';
        Len += Format_SignDec(Out + Len, GeoidSeparation, 4, 1);
        Out[Len++] = 'm';
        Out[Len++] = ' ';
        Len += Format_UnsDec(Out + Len, Speed / 10, 2, 1);
        Out[Len++] = 'k';
        Out[Len++] = 't';
        Out[Len++] = ' ';
        Len += Format_UnsDec(Out + Len, Heading / 10, 4, 1);
        Out[Len++] = 'd';
        Out[Len++] = 'e';
        Out[Len++] = 'g';
        Out[Len++] = '\n';
    ...
    int8_t ReadGGA(NMEA_RxMsg &RxMsg) {
        ...
        if (ReadTime((const char *)RxMsg.ParmPtr(0)) > 0)
            setComplete();
        else
            clrComplete();  // read time and check if same as the RMC says
        FixQuality = Read_Dec1(*RxMsg.ParmPtr(5));
        if (FixQuality < 0) FixQuality = 0;                      // fix quality: 0=invalid, 1=GPS, 2=DGPS
    ...
    int8_t ReadGGA(const char *GGA) {
        ...
        uint8_t Index[20];
        if (IndexNMEA(Index, GGA) < 14) return -2;  // index parameters and check the sum
        if (ReadTime(GGA + Index[0]) > 0)
            setComplete();
        else
            clrComplete();
        FixQuality = Read_Dec1(GGA[Index[5]]);
        if (FixQuality < 0) FixQuality = 0;      // fix quality
    ...
    int8_t ReadGSA(NMEA_RxMsg &RxMsg) {
        ...
        FixMode = Read_Dec1(*RxMsg.ParmPtr(1));
        if (FixMode < 0) FixMode = 0;               // fix mode
    ...
    int8_t ReadGSA(const char *GSA) {
        ...
        uint8_t Index[20];
        if (IndexNMEA(Index, GSA) < 17) return -2;  // index parameters and check the sum
        FixMode = Read_Dec1(GSA[Index[1]]);
        if (FixMode < 0) FixMode = 0;
    ...
    int ReadRMC(NMEA_RxMsg &RxMsg) {
        ...
        if (ReadTime((const char *)RxMsg.ParmPtr(0)) > 0)
            ...
            clrComplete();                                                   // read time and check if same as the GGA says
    ...
    int8_t ReadRMC(const char *RMC) {
        ...
        uint8_t Index[20];
        if (IndexNMEA(Index, RMC) < 12) return -2;  // index parameters and check the sum
        if (ReadTime(RMC + Index[0]) > 0)
            setComplete();
        else
            clrComplete();
    ...
    int8_t Encode(OGN_Packet &Packet) const {
        ...
        if ((FixQuality > 0) && (FixMode >= 2))
            Packet.setFixMode(FixMode - 2);
        else
            Packet.setFixMode(0);
        if (PDOP > 0)
            Packet.EncodeDOP(PDOP - 10);  // encode PDOP from GSA
        else
            Packet.EncodeDOP(HDOP - 10);  // or if no GSA: use HDOP
        int ShortTime = Sec;
        if (FracSec >= 50) {
            ShortTime += 1;
            if (ShortTime >= 60) ShortTime -= 60;
        }
 ...
 private:
    int8_t ReadLatitude(char Sign, const char *Value) {
        int8_t Deg = Read_Dec2(Value);
        if (Deg < 0) return -1;
        int8_t Min = Read_Dec2(Value + 2);
        if (Min < 0) return -1;
        if (Value[4] != '.') return -1;
        int16_t FracMin = Read_Dec4(Value + 5);
        if (FracMin < 0) return -1;
        ...
        // printf("Latitude: %d\n", Latitude);
        return 0;
    }  // Latitude units: 0.0001/60 deg

    int8_t ReadLongitude(char Sign, const char *Value) {
        int16_t Deg = Read_Dec3(Value);
        if (Deg < 0) return -1;
        int8_t Min = Read_Dec2(Value + 3);
        if (Min < 0) return -1;
        if (Value[5] != '.') return -1;
        int16_t FracMin = Read_Dec4(Value + 6);
        if (FracMin < 0) return -1;
        ...
        if (Sign == 'W')
            ...
            return -1;
        return 0;
    }  // Longitude units: 0.0001/60 deg

    int8_t ReadAltitude(char Unit, const char *Value) {
        if (Unit != 'M') return -1;
        return Read_Float1(Altitude, Value);
    }  // Altitude units: 0.1 meter

    int8_t ReadGeoidSepar(char Unit, const char *Value) {
        if (Unit != 'M') return -1;
        return Read_Float1(GeoidSeparation, Value);
    }  // GeoidSepar units: 0.1 meter

    int8_t ReadSpeed(const char *Value) { return Read_Float1(Speed, Value); }  // Speed units: 0.1 knots

    int8_t ReadHeading(const char *Value) { return Read_Float1(Heading, Value); }  // Heading units: 0.1 degree
    ...
    int8_t ReadTime(const char *Value) {
        ...
        int8_t Same = 1;
        Prev = Hour;
        Hour = Read_Dec2(Value);
        if (Hour < 0) return -1;  // read hour (two digits)
        if (Prev != Hour) Same = 0;
        Prev = Min;
        Min = Read_Dec2(Value + 2);
        if (Min < 0) return -1;  // read minute (two digits)
        if (Prev != Min) Same = 0;
        Prev = Sec;
        Sec = Read_Dec2(Value + 4);
        if (Sec < 0) return -1;  // read second (two digits)
        if (Prev != Sec) Same = 0;
        Prev = FracSec;
        if (Value[6] == '.')  // is there a second fraction ?
        {
            FracSec = Read_Dec2(Value + 7);
            if (FracSec < 0) return -1;
        }
        if (Prev != FracSec) Same = 0;
        return Same;
    }  // return 1 when time did not change (both RMC and GGA were for same time)

    int8_t ReadDate(const char *Param) {
        Day = Read_Dec2(Param);
        if (Day < 0) return -1;  // read calendar year (two digits - thus need to be extended to four)
        Month = Read_Dec2(Param + 2);
        if (Month < 0) return -1;  // read calendar month
        Year = Read_Dec2(Param + 4);
        if (Year < 0) return -1;  // read calendar day
        return 0;
    }
 ...
 private:
    ...
    template <class Type>
    static Type Times60(Type X) {
        return ((X << 4) - X) << 2;
    }

    template <class Type>
    static Type Times28(Type X) {
        X += (X << 1) + (X << 2);
        return X << 2;
    }

    template <class Type>
    static Type Times24(Type X) {
        X += (X << 1);
        return X << 3;
    }
};
