...
inline uint8_t NMEA_AppendCheck(char *NMEA, uint8_t Len) {
    return NMEA_AppendCheck((uint8_t *)NMEA, Len);
}
uint8_t NMEA_AppendCheckCRNL(uint8_t *NMEA, uint8_t Len);
inline uint8_t NMEA_AppendCheckCRNL(char *NMEA, uint8_t Len) {
    return NMEA_AppendCheckCRNL((uint8_t *)NMEA, Len);
}
 ...
 public:
    ...
    void ProcessByte(uint8_t Byte)  // pass all bytes through this call and it will build the frame
    {
        ...
        if (Len == 0)              // if data is empty
        {
            ...
            setLoading();
            Check = 0x00;
            Parms = 0;  // set state to "isLoading", clear checksum
        } else          // if not empty (being loaded)
        {
            if ((Byte == '\r') || (Byte == '\n'))  // if CR (or NL ?) then frame is complete
            {
                setComplete();
                if (Len < MaxLen) Data[Len] = 0;
                return;
            ...
            if (Len < MaxLen) {
                Data[Len++] = Byte;
                Check ^= Byte;
            }  // store data but if too much then treat as an error
            else
                Clear();  // if too long, then drop the frame completely
        }
        return;
    }
    ...
    uint8_t isChecked(void) const  // is the NMEA checksum OK ?
    {
        ...
        int8_t HighDigit = HexValue(Data[Len - 2]);
        if (HighDigit < 0) return 0;
        int8_t LowDigit = HexValue(Data[Len - 1]);
        if (LowDigit < 0) return 0;
