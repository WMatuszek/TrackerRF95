...
class UBX_RxMsg  // receiver for the UBX sentences
{
 ...
 private:
    ...
    uint8_t CheckB;
    void CheckPass(uint8_t Byte) {
        CheckA += Byte;
        CheckB += CheckA;
    }  // pass a byte through the checksum
...
class UBX_NAV_TIMEGPS {
 ...
 public:
    ...
    uint8_t Valid(void) const { return (valid & 0x03) == 0x03; }
    uint32_t UnixTime() const { return (iTOW + 10) / 1000 + week * SecsPerWeek + 315964785; }  // http://www.andrews.edu/~tzs/timeconv/timedisplay.php
};
