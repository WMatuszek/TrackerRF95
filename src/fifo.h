...
template <class Type, const size_t Size = 8>  // size must be (!) a power of 2
// like 4, 8, 16, 32, etc.
class VolatileFIFO {
 public:
 ...
 public:
    ...
    size_t isEmpty(void) const { return ReadPtr == WritePtr; }
...
template <class Type, const uint8_t Size = 8>  // size must be (!) a power of 2
// like 4, 8, 16, 32, etc.
class Delay {
 public:
