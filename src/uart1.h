...
int inline UART1_TxDone(void) {
    return USART_GetFlagStatus(USART1, USART_FLAG_TC) != RESET;
}
int inline UART1_TxEmpty(void) {
    return USART_GetFlagStatus(USART1, USART_FLAG_TXE) != RESET;
}
int inline UART1_RxReady(void) {
    return USART_GetFlagStatus(USART1, USART_FLAG_RXNE) != RESET;
}
// int inline UART1_TxEmpty(void)    { return USART1->SR & USART_FLAG_TXE; }

void inline UART1_TxChar(char ch) {
    USART_SendData(USART1, ch);
}
char inline UART1_RxChar(void) {
